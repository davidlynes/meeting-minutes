...
fn detect_and_report_gpu_capabilities() {
    ...
    match target_os.as_str() {
        ...
        "windows" => {
            if cfg!(feature = "cuda") {
                ...
                println!("cargo:warning=⚠️  Windows: Using CPU (OpenBLAS) - GPU acceleration NOT enabled");
        ...
        "linux" => {
            if cfg!(feature = "cuda") {
                ...
                println!("cargo:warning=⚠️  Linux: Using CPU (OpenBLAS) - GPU acceleration NOT enabled");
                println!("cargo:warning=💡 For NVIDIA GPU: cargo build --release --features cuda");
    ...
    if !cfg!(feature = "cuda") && !cfg!(feature = "vulkan") && !cfg!(feature = "hipblas") && target_os != "macos" {
        println!("cargo:warning=📊 Performance: CPU builds are 3-10x slower than GPU builds");
