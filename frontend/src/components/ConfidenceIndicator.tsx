...
export const ConfidenceIndicator: React.FC<ConfidenceIndicatorProps> = ({
  ...
  const getColorClass = (conf: number): string => {
    if (conf >= 0.7) return 'bg-green-500'; // 70-100%: High confidence
    if (conf >= 0.6) return 'bg-yellow-500'; // 60-69%: Good confidence
    if (conf >= 0.5) return 'bg-orange-500'; // 50-59%: Medium confidence
